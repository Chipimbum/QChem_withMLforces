...
class Parser:
    ...
    def parse_file(self):
        ...
        while self._search_line(START_OF_ORIENTATION):
            ...
            if not orientations or not energy \
                    or len(orientations) != self._atom_num : return
            self._write_atom_num()
            self._write_energy(energy)
            self._write_orientation(orientations)
            #if START_OF_GRADIENT: return #This is if I want only one geometry at a time
    ...
    def _write_orientation(self, orientation_lines):
        """
        Write the orientation of the atoms matrix to the output file
        """
        for line in range(len(orientation_lines)):
            self._output_file.write(orientation_lines[line] + COL_SEPARATOR + NEW_LINE)
